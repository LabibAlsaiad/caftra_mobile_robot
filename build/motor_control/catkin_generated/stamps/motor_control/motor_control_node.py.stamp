...
def control_motors(linear_x, angular_z):
    ...
    # Set motor A
    if motor_a_speed < 0:
        pwm_a1.ChangeDutyCycle(abs(motor_a_speed) * 100)
        pwm_a2.ChangeDutyCycle(0)
    elif motor_a_speed > 0:
        pwm_a1.ChangeDutyCycle(0)
        pwm_a2.ChangeDutyCycle(abs(motor_a_speed) * 100)
    else:
        pwm_a1.ChangeDutyCycle(0)
        pwm_a2.ChangeDutyCycle(0)
    ...
    # Set motor B
    if motor_b_speed < 0:
        pwm_b1.ChangeDutyCycle(abs(motor_b_speed) * 100) 
        pwm_b2.ChangeDutyCycle(0)
    elif motor_b_speed > 0:
        pwm_b1.ChangeDutyCycle(0)
        pwm_b2.ChangeDutyCycle(abs(motor_b_speed) * 100)
    else:
        pwm_b1.ChangeDutyCycle(0)
        pwm_b2.ChangeDutyCycle(0)
